...
size_t sys_pwrite64(int fd, const void *buf, size_t count, off_t offset) {
	...
	if(fd & LWIP_FD_BIT) {
		LOG_ERROR("pwrite64 not supported on a socket\n");
		return -ENOSYS;
	}
	...
	if(prev_offset == (uint64_t)-1) {
		LOG_ERROR("pwrite64: cannot get  offset\n");
		return -1;
	}

	/* Set the offset */
	if(sys_lseek(fd, offset, SEEK_SET) == (uint64_t)-1) {
		LOG_ERROR("pwrite64: cannot set offset\n");
		ret = -1;
		goto out;
	}
	...
	if(sys_lseek(fd, prev_offset, SEEK_SET) == (uint64_t)-1) {
		LOG_ERROR("pwrite64: cannot set back offset\n");
		ret = -1;
		goto out;
	}
